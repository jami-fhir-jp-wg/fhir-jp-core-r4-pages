...
FHIR実装ガイドで採用している「CodeSystem」や「Identifierに対するSystem値」に対するURIについて、可能なものからOID形式からURL形式への移行を進めている。URL形式はWebにおける標準的な識別子であり、人間にとっての可読性、相互運用性やアクセシビリティの向上になど優れている点がある。

現時点では既存システムとの互換性を維持し円滑な移行を促すため、URL形式の利用は必須としておらず、NamingSystemリソースに記載されるOID形式の記載も許容している。しかし、HL7 FHIR標準Validatorやその他のFHIRライブラリはNamingSystemリソースの内容まで参照していないことが多い。そのため実装者は、1)提供されるPackageファイルをNamingSystemリソースを参照し、OID形式への逆変換を行う、2)OIDベースのシステムからデータを外部に入出力する際に、URL形式に変換するなどの工夫を行うことで回避をすること。

将来的なFHIR実装ガイドの改訂においては、URL形式の利用を必須とする方向で検討を進める予定である。そのため、新規システムの構築においては、可能な限りURL形式を採用することが推奨される。

#### URL形式のパッケージを利用した際の影響点

FHIRにおけるOID形式からURL形式への変更は、既存システムに以下の影響を与える。

* **Validatorに対してエラーが発生する**
 OIDを利用して作成されたリソースはValidationチェックに引っかかる。これはコードシステム、バリューセット、拡張定義、プロファイルなど、様々な箇所で発生する可能性がある。
* **データマイグレーションが必要となる場合がある。**
 既存データにOID形式で格納された識別子がある場合は、URL形式への変換が必要となる。
...
| ＊ | 投与経路 HL7 V2(使用者定義表0162) | urn:oid:1.2.392.100495.20.2.34 | http://jpfhir.jp/fhir/core/CodeSystem/route-codes |
...
| ＊ | MEDISー医薬品基準番号（ＨＯＴ９） | urn:oid:1.2.392.200119.4.403.1urn:oid:1.2.392.100495.20.2.74 | http://medis.or.jp/CodeSystem/master-HOT9 |
...
| ＊ | 医療機関コード（７桁） | urn:oid:1.2.392.100495.20.3.23 | http://jpfhir.jp/fhir/core/mhlw/IdSystem/MedicalOrganizationID |
...
| ＊ | 処方箋 ID | urn:oid:1.2.392.100495.20.3.11.y枝番仕様あり | URL表記は使用しない |
|   | 調剤結果 ID | urn:oid:1.2.392.100495.20.3.12.y枝番仕様あり | URL表記は使用しない |
| ＊ | 麻薬施用者免許番号 | urn:oid:1.2.392.100495.20.3.32.y枝番仕様あり | URL表記は使用しない |
|   | 薬剤師名簿登録番号 | urn:oid:1.2.392.100495.20.3.33.y枝番仕様あり | URL表記は使用しない |
| ＊ | 医師を特定する ID | urn:oid:1.2.392.100495.20.3.41.x枝番仕様あり | URL表記は使用しない |
| ＊ | 歯科医師を特定する ID | urn:oid:1.2.392.100495.20.3.42.x枝番仕様あり | URL表記は使用しない |
|   | 薬剤師を特定する ID | urn:oid:1.2.392.100495.20.3.43.x枝番仕様あり | URL表記は使用しない |
| ＊ | 患者を特定する ID | urn:oid:1.2.392.100495.20.3.51.x枝番仕様あり | URL表記は使用しない |
